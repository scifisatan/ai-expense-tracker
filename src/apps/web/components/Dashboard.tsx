...
const Dashboard = ({ email, onLogout }: { email: string | null; onLogout: () => void }) => {
  ...
  const [sidebarOpen, setSidebarOpen] = useState(() => {
    // Default open on larger displays (>1200px), closed on tablet/1024px laptops
  ...
  return (
    <div className="flex min-h-dvh bg-background">
      {/* ─────────────────────────────────────────────────────────────
          Desktop Left Sidebar (Collapses between full and icon-only rail)
          ───────────────────────────────────────────────────────────── */}
      <aside
        className={cn(
          "hidden shrink-0 flex-col justify-between border-r bg-card/60 transition-all duration-300 md:flex",
          sidebarOpen ? "w-64 p-4 lg:w-72" : "w-16 p-2 items-center"
        )}
      >
        <div className="space-y-6 w-full">
          {/* Brand Header & Toggle Action */}
          {sidebarOpen ? (
            <div className="flex items-center justify-between px-2 py-1.5">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h1 className="text-base font-semibold tracking-tight text-foreground">Pacer</h1>
            ...
            <div className="flex flex-col items-center gap-3 pt-1">
              <Button
                ...
                className="size-10 rounded-2xl bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm"
                  ...
                  {section.items.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "flex items-center rounded-2xl text-left text-sm font-semibold transition-all",
                          sidebarOpen
        ...
        <div className={cn("border-t pt-3 w-full", sidebarOpen ? "space-y-3" : "flex flex-col items-center space-y-2")}>
          {sidebarOpen ? (
            <>
