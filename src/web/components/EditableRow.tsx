...
const EditableRow = ({
    ...
    const dateStr = useMemo(() => {
        try {
            return new Date(tx.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: '2-digit' });
        } catch { return tx.createdAt; }
    }, [tx.createdAt]);
    ...
    if (!editing) {
        return (
            <tr>
                ...
                <td><span className={tx.note ? 'note-cell' : 'note-empty'}>{tx.note || 'no note'}</span></td>
                <td className="date-cell">{dateStr}</td>
                <td style={{ display: 'flex', gap: 4 }}>
                    <button className="btn-edit" onClick={() => setEditing(true)}>Edit</button>
                    <button className="btn-delete" onClick={() => { if (confirm('Delete this transaction?')) onDelete(); }}>Delete</button>
                </td>
            </tr>
        );
    }
    ...
    return (
        <tr className="edit-row">
            ...
            <td>
                <input className="inline-input" value={amount} onChange={e => setAmount(e.target.value)} style={{ width: 120 }} />
            </td>
            <td>
                <input className="inline-input" value={note} onChange={e => setNote(e.target.value)} style={{ minWidth: 140 }} />
            </td>
            <td className="date-cell">{dateStr}</td>
            <td style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                <button className="btn-save" onClick={save} disabled={saving}>{saving ? '…' : 'Save'}</button>
                <button className="btn-cancel" onClick={() => { setEditing(false); setType(tx.type); setAmount(tx.amount.toString()); setNote(tx.note ?? ''); }}>✕</button>
            </td>
        </tr>
    );
};
